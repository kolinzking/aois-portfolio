# Phase 0 Contents

This is the central navigation file for Phase 0.

If you want to find a topic quickly, start here.

## Start Here

1. [Phase 0 Introduction](00-introduction.md)
2. [Study Pacing](../STUDY-PACING.md)

## Phase 0 Goal

Build the engineering substrate that every later AOIS phase depends on:

- Linux and terminal fluency
- bash scripting
- Git discipline
- networking and HTTP
- Python service foundations
- FastAPI basics
- LLM fundamentals
- persistence foundations

## Version Map

| Version | Topic | Primary Build |
|---|---|---|
| `v0.1` | Linux essentials | `scripts/sysinfo.sh` |
| `v0.2` | Bash and rule-based interpretation | `scripts/log_analyzer.sh` |
| `v0.3` | Git as engineering memory | repo history as proof |
| `v0.4` | Networking and HTTP | manual API inspection drills |
| `v0.5` | Python core for systems work | typed backend utilities |
| `v0.6` | FastAPI without AI | mock AOIS API |
| `v0.7` | LLM fundamentals | raw model call |
| `v0.8` | Postgres and AOIS memory | incident and analysis schema |

## Current Authored Lessons

### `v0.1`

- [CONTENTS.md](v0.1/CONTENTS.md)
- [introduction.md](v0.1/introduction.md)
- [notes.md](v0.1/notes.md)
- [lab.md](v0.1/lab.md)
- [runbook.md](v0.1/runbook.md)
- [benchmark.md](v0.1/benchmark.md)
- [failure-story.md](v0.1/failure-story.md)
- [summarynotes.md](v0.1/summarynotes.md)
- [next-version-bridge.md](v0.1/next-version-bridge.md)
- [looking-forward.md](v0.1/looking-forward.md)

### `v0.2`

- [CONTENTS.md](v0.2/CONTENTS.md)
- [introduction.md](v0.2/introduction.md)
- [notes.md](v0.2/notes.md)
- [lab.md](v0.2/lab.md)
- [runbook.md](v0.2/runbook.md)
- [benchmark.md](v0.2/benchmark.md)
- [failure-story.md](v0.2/failure-story.md)
- [summarynotes.md](v0.2/summarynotes.md)
- [next-version-bridge.md](v0.2/next-version-bridge.md)
- [looking-forward.md](v0.2/looking-forward.md)

### `v0.3`

- [CONTENTS.md](v0.3/CONTENTS.md)
- [introduction.md](v0.3/introduction.md)
- [notes.md](v0.3/notes.md)
- [lab.md](v0.3/lab.md)
- [runbook.md](v0.3/runbook.md)
- [benchmark.md](v0.3/benchmark.md)
- [failure-story.md](v0.3/failure-story.md)
- [summarynotes.md](v0.3/summarynotes.md)
- [next-version-bridge.md](v0.3/next-version-bridge.md)
- [looking-forward.md](v0.3/looking-forward.md)

### `v0.4`

- [CONTENTS.md](v0.4/CONTENTS.md)
- [introduction.md](v0.4/introduction.md)
- [notes.md](v0.4/notes.md)
- [lab.md](v0.4/lab.md)
- [runbook.md](v0.4/runbook.md)
- [benchmark.md](v0.4/benchmark.md)
- [failure-story.md](v0.4/failure-story.md)
- [summarynotes.md](v0.4/summarynotes.md)
- [next-version-bridge.md](v0.4/next-version-bridge.md)
- [looking-forward.md](v0.4/looking-forward.md)

### `v0.5`

- [CONTENTS.md](v0.5/CONTENTS.md)
- [introduction.md](v0.5/introduction.md)
- [notes.md](v0.5/notes.md)
- [lab.md](v0.5/lab.md)
- [runbook.md](v0.5/runbook.md)
- [benchmark.md](v0.5/benchmark.md)
- [failure-story.md](v0.5/failure-story.md)
- [summarynotes.md](v0.5/summarynotes.md)
- [next-version-bridge.md](v0.5/next-version-bridge.md)
- [looking-forward.md](v0.5/looking-forward.md)

### `v0.6`

- [CONTENTS.md](v0.6/CONTENTS.md)
- [introduction.md](v0.6/introduction.md)
- [notes.md](v0.6/notes.md)
- [lab.md](v0.6/lab.md)
- [runbook.md](v0.6/runbook.md)
- [benchmark.md](v0.6/benchmark.md)
- [failure-story.md](v0.6/failure-story.md)
- [summarynotes.md](v0.6/summarynotes.md)
- [next-version-bridge.md](v0.6/next-version-bridge.md)
- [looking-forward.md](v0.6/looking-forward.md)

### `v0.7`

- [CONTENTS.md](v0.7/CONTENTS.md)
- [introduction.md](v0.7/introduction.md)
- [notes.md](v0.7/notes.md)
- [lab.md](v0.7/lab.md)
- [runbook.md](v0.7/runbook.md)
- [benchmark.md](v0.7/benchmark.md)
- [failure-story.md](v0.7/failure-story.md)
- [summarynotes.md](v0.7/summarynotes.md)
- [next-version-bridge.md](v0.7/next-version-bridge.md)
- [looking-forward.md](v0.7/looking-forward.md)

### `v0.8`

- [CONTENTS.md](v0.8/CONTENTS.md)
- [introduction.md](v0.8/introduction.md)
- [notes.md](v0.8/notes.md)
- [lab.md](v0.8/lab.md)
- [runbook.md](v0.8/runbook.md)
- [benchmark.md](v0.8/benchmark.md)
- [failure-story.md](v0.8/failure-story.md)
- [summarynotes.md](v0.8/summarynotes.md)
- [next-version-bridge.md](v0.8/next-version-bridge.md)
- [looking-forward.md](v0.8/looking-forward.md)

## When You Feel Lost

If you are trying to find where a topic lives:

- Linux and the terminal: start with [`v0.1`](v0.1/notes.md)
- bash scripting and brittle rules: start with [`v0.2`](v0.2/notes.md)
- Git and history discipline: start with [`v0.3`](v0.3/notes.md)
- networking and HTTP inspection: start with [`v0.4`](v0.4/notes.md)
- Python foundations: start with [`v0.5`](v0.5/notes.md)
- FastAPI service boundary: start with [`v0.6`](v0.6/notes.md)
- raw model call anatomy: start with [`v0.7`](v0.7/notes.md)
- persistence and schema design: start with [`v0.8`](v0.8/notes.md)

## Phase Boundary Files

- [Introduction](00-introduction.md)
- [Looking Forward](looking-forward.md)
