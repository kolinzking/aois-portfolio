# v0.2 Contents

Authoring status: authored

## Start Here

- [introduction.md](introduction.md)
- [notes.md](notes.md)

## Topic Jumps

- [What This Builds](notes.md#what-this-builds)
- [Why Bash Exists](notes.md#why-this-exists)
- [Core Bash Building Blocks](notes.md#core-concepts)
- [Build](notes.md#build)
- [Ops Lab](notes.md#ops-lab)
- [Break Lab](notes.md#break-lab)
- [Benchmark](notes.md#benchmark)
- [Mastery Checkpoint](notes.md#mastery-checkpoint)

## Practice Files

- [lab.md](lab.md)
- [runbook.md](runbook.md)
- [benchmark.md](benchmark.md)
- [failure-story.md](failure-story.md)

## Memory Files

- [summarynotes.md](summarynotes.md)
- [next-version-bridge.md](next-version-bridge.md)
- [looking-forward.md](looking-forward.md)

## Self-Paced Path

1. read [introduction.md](introduction.md)
2. work through [notes.md](notes.md) in order
3. build the script and run the labs
4. use [runbook.md](runbook.md) if the script fails
5. read [summarynotes.md](summarynotes.md)
6. use [next-version-bridge.md](next-version-bridge.md) to move into `v0.3`
<!-- AOIS-NAV-START -->
---

## Navigation

- Reading order: [AOIS Reading Order](../../READING-ORDER.md)
- Previous: [v0.2 Start Here](00-start-here.md)
- Next: [v0.2 Introduction](introduction.md)
<!-- AOIS-NAV-END -->
