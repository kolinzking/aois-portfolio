# v23 Contents

Authoring status: authored

## Start Here

1. [introduction.md](introduction.md)
2. [notes.md](notes.md)
3. [lab.md](lab.md)
4. [runbook.md](runbook.md)
5. [benchmark.md](benchmark.md)

## Topic Jumps

- Orchestration loop policy: [notes.md](notes.md)
- Loop simulation: [lab.md](lab.md)
- Operational handling: [runbook.md](runbook.md)
- Benchmark target: [benchmark.md](benchmark.md)
- Failure mode: [failure-story.md](failure-story.md)
- Bridge to v23.5: [next-version-bridge.md](next-version-bridge.md)

## Self-Paced Path

1. Inspect `agentic/aois-p/stateful-orchestration.plan.json`.
2. Run the validator.
3. Run the simulator.
4. Explain the ordered decision precedence.
5. Break one stop guard and predict the validation or simulation failure.
6. Restore the plan and read the bridge into v23.5.
<!-- AOIS-NAV-START -->
---

## Navigation

- Reading order: [AOIS Reading Order](../../READING-ORDER.md)
- Previous: [v23 Start Here](00-start-here.md)
- Next: [v23 Introduction](introduction.md)
<!-- AOIS-NAV-END -->
